<script lang="ts">
  ...
  const { form: initialForm, profile, user } = data;
    ...
    <div class="p-5 mb-5 border border-gray-300 shadow-sm card bg-base-100">
      ...
      <div class="flex items-center justify-between">
        {#if user?.appMetadata?.provider === "email"}
          ...
          <button
            class="relative ml-4 btn btn-link group"
            onclick={() => handleSubItemClick("PasswordUpdate")}
          >
            <i class="fas fa-edit"></i>
            <span
              class="absolute px-2 py-1 -mb-2 text-xs text-white transition-opacity duration-200 transform -translate-x-1/2 bg-gray-800 rounded opacity-0 bottom-full left-1/2 group-hover:opacity-100"
        ...
        {:else}
          <p class="flex-1 overflow-hidden text-sm text-gray-500">
            Password change is not available because you logged in using
            Facebook or Google. Please contact your admin to change your
            password.
          </p>
        {/if}
      </div>
